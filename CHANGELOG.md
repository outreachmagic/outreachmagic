...
- **Contacts "Tags" column showed `undefined`** — `dashboard.html` declared `function renderTagChips` twice at top level: the table-cell renderer that takes a row, and the tag-filter-bar renderer that takes nothing. Hoisting means the second wins for the whole script, so both tables using the Tags column called the filter-bar renderer, printed its `undefined` return into every row, and re-rendered the filter bar once per row as a side effect. The filter-bar one is now `renderTagFilterBar()`. New `tests/test_dashboard_html_hygiene.py` fails on any duplicate top-level `function` declaration — there is no bundler in front of this file, so this class of bug ships silently.
- **Lead panel's "Existing tag…" dropdown was always empty** — it fetched `/api/tags` with `api()`, which does not append `?workspace=`; the route is workspace-scoped and rejects the request, and a `.catch(() => {})` swallowed it. Now uses `get()`. The hygiene test also fails on any `api()` call to a workspace-scoped route.
- **Dashboard pull finished successfully and then reported "[Errno 32] Broken pipe"** — the pull worker redirected stdout into the console log but left stderr alone, and `pipeline_sync._relay_log` wrote to stderr with a bare `print`. Under a harness that closes stderr the first diagnostic raised `BrokenPipeError`, which `SyncManager._run` catches as `OSError` and files as a failed run — after the log had recorded a complete successful pull. `_relay_log` now goes through `progress.warn` (already `BrokenPipeError`-safe, and whose docstring describes this exact failure), and the pull worker redirects stderr too.
- **Export said "Exporting…" and then nothing, taking the database and the app with it** — `full` is ~30 correlated subqueries per row, and `export_to_csv` materialized the entire result set (message bodies included) into a list of dicts before writing a byte, inline on the request thread with no timeout and a toast that expired after 2.8s. Rows now stream from the cursor straight into the writer, and the dashboard export runs as a background job through `SyncManager` — the same single-flight lock, status endpoint and console drawer that pull and push use, with per-2,000-row progress and the file path in the completion line. The CLI stays synchronous.
- **`import-profiles` renamed people** — a sheet of `lead_id` + personalized columns has no name column, so `personalized_first_name` was promoted into `leads.name`: "Brian Williams" → "Brian". Promotion no longer happens when the row identifies an existing lead, and `--mode personalization-only` makes it structurally impossible for a whole file.
- **`import-profiles --record-type` was documented but did not exist** — `SKILL.md` told you to pass it and argparse rejected it. `import_profiles()` had accepted the parameter all along; only the CLI flag was missing.
- **Company stubs imported as fake people** — `looks_like_company_name` is a whitelist, so dealership names carrying no corporate token (`K L M of Riverton`, `Corwin Vance`, `Brennt of Ashgrove`) failed it while `K L M of Fairhaven, Inc.` from the same import passed; 30 of 773 rows in one manufacturer import became `contact` rows that then appeared as people and burned TryKitt credits. Three fixes, because one is not enough: known directory sources (`COMPANY_DIRECTORY_SOURCES` — `mercedes-benz-official`, anything ending `-official`, `google_maps`) skip the whitelist entirely; the tokenizer now splits on `,` `.` `(` `)` so `Rensfield Autowerks,Inc` sees its own "Inc"; and nine dealership tokens were added. Email-finder eligibility also excludes any `contact` row whose name equals its company with no email/title/LinkedIn, so a missed classification costs no credits regardless. The strict whitelist is unchanged for scraped and unknown sources — it exists to protect sole traders whose company is their own name, and nothing here changes what happens to those rows.
- **`personalize-set --batch --json /path/to/file.json` failed as `JSONDecodeError: Expecting value`** — it `json.loads`-ed the path. `load_json_array_from_cli` (which already supported `--file`) is now wired into `personalize-set`, `company-personalize-set` and `serper-apply`, gained stdin support (`--json -`), and rejects a value that is obviously a path with a message naming the right flag. `tag bulk` gains `--file` taking one lead id per line or a JSON array.
- **Suppressed contacts vanished from exports silently** — "give me everyone with this tag" returned 1,873 of 1,875 with nothing to say why. The export result now carries `suppressed_excluded` (count plus lead ids), and the dashboard console reports it.
- **Two independent tag filters on the contacts page** — the chip bar wrote `tags_any`/`tags_all`/`tags_none` while stats drill-through and lead-panel chips wrote a scalar `tag`, rendered elsewhere and cleared separately. `tagChips` is now the single source of truth; a `tag` patch becomes a chip. Chips also clear on workspace switch (they are workspace-scoped, and carrying them across reads as an empty workspace).

### Changed

- **`first_name` and `last_name` are no longer export columns.** There is no such column on `leads`; both were a SQL split of `l.name` on the first space, which mangled every name that isn't exactly two words — "Mary Anne Okonkwo" exported a last name of "Anne Okonkwo", a mononym exported an empty one, and a company placeholder was shredded into a fake person. A wrong first name in a mail merge is worse than an absent one. Use `name`, or `personalized_first_name` for a curated merge value (unaffected — it is a stored value, not a derived one). Asking for either column returns an error saying what to use instead.
- **Suppression offers one domain type, not two.** `company_domain` already matched the company's domain OR the contact's email domain, making it a superset of `email_domain` — so "which one blocks acme.com?" had a wrong answer available. The picker now offers `Domain`; both types remain in the enum so existing rules keep working, and the table labels them `domain` / `domain (email only)`.
- **Event-history messages expand in place.** The old shape stacked preview, then a button, then the full body below it, so an expanded message read as its first two lines twice followed by everything. One element, two states, shared by the lead and company timelines.
- **Export column picker groups by scope** — Contact · Messages · Contact personalization · Company personalization, so "is this the contact's or the account's?" is answerable at the point of choosing.

### Added

- **Personalization scope registry** (`personalization_fields` table, `pipeline.py personalization-fields [--scope] [--values]`) — a personalization field belongs to exactly one scope, and until now scope was guessed per write by a nine-word name-shape heuristic and then never reported. Nothing downstream could answer "will this column land on the contact or the account?", which is why the export picker was unreadable, why `full` silently omitted the most-populated personalization table in the database, and why one sheet could write eleven different `company_name` values without complaint. First write to a scope claims the name; a contradicting write is refused rather than silently landing in a second table. Backfilled from what is on disk — on the live database exactly one name (`company_name`, 101 lead rows against 53,255 company rows) exists in both scopes, and the majority rule is reported rather than applied silently. `--values` shows the distinct values each field already holds, so reusing `icp_segment` is an informed choice rather than a collision.
- **Company personalization is exportable** (`company_personalized_<field>`) — `export --preset full` now includes it, and a bare `personalized_company_name` resolves through to company scope, so the column every existing preset and script already asks for returns the value it always should have. Import accepts the same prefix as an explicit scope declaration.
- **`import-profiles` reports where every personalization column went** — `summary.personalization_routing` per column with `explicit` / `registry` / `heuristic` provenance, and `personalization_routing_guessed` naming the ones nothing had claimed.
- **Per-company personalization conflict detection** — company personalization is one value per company but a sheet is per lead, so the same company could arrive with different values and the last row silently won. Company writes are now batched after the loop, conflict-checked once, and abort by default naming the offending companies; `--company-conflict last-wins|skip` are the escape hatches.
- **Pre-import validation** — runs on every import, not just `--dry-run`: rows that will not match, values that would be overwritten (old → new), and company conflicts, all before anything is written. Fifteen non-existent lead ids used to be visible only as per-row errors afterwards.
- **`import-profiles --mode personalization-only`** — rows must match an existing lead; nothing is created, no profile column is touched, and unmatched rows come back as a list instead of becoming ghost leads.
- **`companies.category`** — the account classification (`google_category`) denormalized off the placeholder lead's personalization, so segmenting contacts by it is a `WHERE` and not a three-hop join. Backfilled; exportable as `company_category`.
- **Test-data flag** (`leads.is_test`, `import-profiles --test`, `pipeline.py test-leads list|suggest|set`) — excluded from lists, counts, exports and bulk actions by default, same discipline as suppression: absent means excluded, so an operator who forgets the filter gets the safe answer. Auto-detection is deliberately narrow (RFC 2606 `example.com`/`.org`/`.net` addresses only): on the live database nine of the seventeen rows matching `%test%` are real people at a real dealership imported under a badly-named source, and flagging them would have hidden real contacts from every export. `test-leads suggest` reports those for a human instead.
- **Filter an export by a personalization value** (`export --personalized icp_segment='mercedes franchise'`, repeatable) — added to `lead_filter_clause`, not to a private filter set, so the contacts list gets it too and the one-filter-set invariant holds. Resolves lead or company scope from the registry, so the caller names the field, not the table.
