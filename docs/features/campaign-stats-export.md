...
> **Feature:** `pipeline.py sheets export --template campaign-stats`
> Generates a multi-sheet Google Workbook with all campaign performance data from Outreach Magic's local SQLite database.

---

## Overview

This feature produces a **3-sheet Google Workbook** that gives anyone (client, team lead, operations manager) an instant view of every campaign's health — send volume, reply rates, OOO detection, lead sentiment, and conversion funnels — in a format that's easy to scan, sort, and present.
...
pipeline.py sheets export --template campaign-stats --workspace popcam

# Custom time window
pipeline.py sheets export --template campaign-stats --workspace popcam --since 30d

# All-time (no time filter)
pipeline.py sheets export --template campaign-stats --workspace popcam --since all

# Custom share email
pipeline.py sheets export --template campaign-stats --workspace popcam --share-email client@example.com

# JSON preview (debug / review before sheet creation)
pipeline.py sheets export --template campaign-stats --workspace popcam --since 14d --dry-run --json
```
...
| `created_at` | Event timestamp — used for time-window filtering |
...
```
Campaign | Status | Sent | Delivered | Bounced | Bounce % | Total Replies | OOO | Manual | Reply % | LI Connects | LI Accepts | Accept % | LI Messages | LI Replies | Interested | Not Interested | Sentiment Rate | Last Activity
```
...
| 9 | **Manual** | `Total Replies - OOO` | Computed column. Human replies only. |
...
Or better: create the funnel tables side-by-side on the same sheet, one campaign per column block.
...
Make 3-4 database queries:

1. **Campaign Overview** — one wide query that joins all event-type CTEs (as shown in Sheet 1 SQL above)
2. **Sentiment Distribution** — the `ranked_status` CTE query grouped by campaign and sentiment
3. **Per-campaign funnel data** — query all active campaign funnels in one pass, or iterate campaigns (small workspaces: iterate; large: batch query)
4. **(Optional) Campaign list** — for status detection, query `created_at` range per campaign to identify paused vs exhausted
...
Each sheet is an array of rows. The first row is the header.
...
```json
{
  ...
  "workspace": "popcam",
  "since": "14d",
  "title": "Popcam Campaign Stats — Last 14 Days",
  "sheets": [
    {
...
### Step 5: Sheet-level formatting (nice-to-have)

If the sheet renderer supports it:

1. **Freeze header row** on all three sheets
...
If a campaign has zero events in the time window, still include it in the Campaign Overview with `sent = 0` and status = `"paused"` or `"exhausted"`. Do not omit it — the user needs to know campaigns exist that aren't producing activity.

### Campaigns with no email sends

Some campaigns may have LinkedIn activity but no email sends. Include them in the Overview with email columns empty (`—`) but LinkedIn columns populated.
...
| Campaign | Status | Sent | Delivered | Bounced | Bounce % | Total Replies | OOO | Manual | Reply % | LI Connects | LI Accepts | Accept % | Interested | Not Int. | Sentiment Rate | Last Activity |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
