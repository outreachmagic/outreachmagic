...
description: >
  ...
  segment performance, and reply copy insights. Use when the user asks about
  pipeline, leads, outreach, campaigns, sequencer data, or says show pipeline.
---

# Outreach Magic — Pipeline Visibility for Cursor

The simplest pipeline tracker. Auto-logs every outreach action to a local
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/server.py
# http://localhost:3100
```
...
- **Read before connect.** See repo root [SECURITY.md](https://github.com/outreachmagic/cursor-skill/blob/main/SECURITY.md) for full data boundaries and vulnerability reporting.
