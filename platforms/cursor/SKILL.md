...
# Outreach Magic — Pipeline Visibility for Cursor

The simplest pipeline tracker. Cursor Agent auto-logs every outreach action to a local
SQLite database. Free forever. Connect Smartlead, Heyreach, Instantly via paid relay.

Invoke manually in Agent chat with `/outreachmagic`, or ask about your pipeline in plain English.

Database: `~/.cursor/skills/outreachmagic/databases/outreachmagic.db`
Config (single source of truth): `~/.cursor/skills/outreachmagic/config/outreachmagic_config.json`
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py version
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py pull
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show
```
...
- Unlimited agent-originated tracking
...
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py version
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --id 1
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --email j@acme.com
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py stats
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py campaigns
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py copy-insights --lead-status interested --json
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py import-profiles --file leads.csv
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export-local
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export-local --file changes.csv
```
...
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py status

# List all connections with webhook URLs and 30-day event counts
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connections
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connections --json

# Generate a webhook URL for a new platform
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connect-platform --platform smartlead

# Remove a platform connection (webhook URL stops working)
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py disconnect-platform --platform smartlead
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py disconnect-platform --platform smartlead --yes
```
...
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export-local --file local_changes.csv

# Filter to a specific workspace
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export-local --workspace leadgenph

# Include all leads (not just locally-created)
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export-local --all
```

**Push to relay for cross-platform sync:**

```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py sync
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py campaigns
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py campaigns --json
```
...
OutreachMagic stores each webhook as an event. **Interested / not interested / sentiment come from label webhooks**, not from reply webhooks alone. OOO is classified as **auto-reply** (metadata flag, query with `--auto-reply true`). Bounces set event sentiment `invalid` but **do not** auto-move the lead to stage `lost` (use `--sentiment invalid` to find them).

After `pull`, filter the pipeline by **current** status (latest status-bearing event per lead):

```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --sentiment positive
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --sentiment invalid
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --auto-reply true
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --lead-status interested --json
```

Filter by date (created or updated on/after a date):

```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --since today
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py show --since 2026-05-26 --json
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py lead-table --workspace acme_corp --since today --json
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py copy-insights --lead-status interested
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py copy-insights --lead-status interested --json
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export --workspace acme_corp --tag nace --format csv
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py export --workspace acme_corp --since today --format json
```
...
```bash
rm ~/.cursor/skills/outreachmagic/databases/outreachmagic.db
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py init
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py pull --full
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --id 1
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --email jane@acme.com
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --name "Jane Doe"
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --id 1 --json
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py add-lead \
...
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py verify-email \
  --lead-id 5 --status valid --source zerobounce

# Batch verify from JSON
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py verify-email --batch \
  --json '[{"lead_id":5,"status":"valid","source":"zerobounce"}]'

# Check verification status for a lead
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py verify-status --lead-id 5
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py verify-status --email j@acme.com

# List leads needing verification
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py verify-pending --limit 50 --json
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py merge-leads --keep 12 --merge 34
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py merge-leads \
  --email j@acme.com --linkedin linkedin.com/in/janedoe
```

```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py history --linkedin linkedin.com/in/janedoe
```
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py log-event \
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py update-stage \
...
```bash
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connect-platform --platform smartlead
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connect-platform --platform instantly
python3 ~/.cursor/skills/outreachmagic/scripts/pipeline.py connections
```
...
- **Local-first data.** Pipeline leads, events, and campaign stats live in `~/.cursor/skills/outreachmagic/databases/outreachmagic.db` on your machine.
...
- **Read before connect.** See repo root [SECURITY.md](https://github.com/outreachmagic/cursor-outreachmagic/blob/main/SECURITY.md) for full data boundaries and vulnerability reporting.
