#!/usr/bin/env python3
"""
Re-push local timeline events to relay with campaign names on each event_log.

Safe re-upload (after worker deployment with event_id upsert):
  1. Purge stale agent event_log rows in the relay DB (no event_id / missing campaign).
