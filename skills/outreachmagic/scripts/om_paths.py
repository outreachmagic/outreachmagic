"""
Skill install paths. Resolves data root from user choice (not directory scanning).
...
This avoids the duplicate-DB problem caused by ``npx skills add`` copying the
skill to every agent directory on the machine.
"""
...
def _config_path_for_agent_dir(agent_dir: str) -> Path:
    """Return the expected config path for a given agent directory name."""
    return Path.home() / agent_dir / "skills" / SKILL_NAME / "config" / "outreachmagic_config.json"
...
def check_duplicate_installs() -> list[dict]:
    ...
    Scans known agent directories and returns entries for every
    installation that is **not** the active install directory and is a
    real copy (not a symlink).  This is pure detection — the user chooses
...
def save_sheets_export_record(
    ...
    **extra_filters: Any,
    ...
    Stores export filters so ``review sync`` can re-use the same scope
    when building its baseline.
    """
...
def find_sheets_export_record(sheet_id: str) -> Optional[dict[str, Any]]:
    """Look up a stored export record by ``sheet_id``.

    Returns the full payload dict (including ``filters``) or ``None``
    when no matching record is found.
    """
    sheets_dir = get_sheets_dir()
    if not sheets_dir.is_dir():
        return None
    clean_sid = sheet_id.strip()
    for fpath in sorted(sheets_dir.iterdir()):
        ...
        try:
            ...
            continue
        if str(data.get("sheet_id", "")).strip() == clean_sid:
            return data
    return None
