...
class ApiKeysCliTests(unittest.TestCase):
    ...
    def test_api_keys_json_no_keys(self):
        ...
        serper = next((item for item in payload["providers"] if item["provider"] == "serper"), None)
        self.assertIsNotNone(serper, "serper provider not found in report")
        # Either no keys configured, or keys exist but either way a valid report
        status = serper.get("status", "configured")
        self.assertIn(status, ("no_keys", "never_used", "configured"))
