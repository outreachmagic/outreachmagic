...
Covers:
  ...
  - Daily breakdown multi-day split
  - Metadata and settings note
"""
...
def run_tests():
    ...
    check("alpha interested=1", ov["rows"][0][2] == 1)
    check("alpha not_interested mdash", ov["rows"][0][3] == "—")
    check("alpha sent=3", ov["rows"][0][4] == 3)
    check("alpha delivered=2", ov["rows"][0][5] == 2)
    check("alpha bounced=1", ov["rows"][0][6] == 1)
    check("alpha bounce% 33.3%", str(ov["rows"][0][7]).startswith("33"))
    check("alpha replies=2", ov["rows"][0][8] == 2)
    check("alpha LI connects=0 (mdash)", ov["rows"][0][12] == "—")
    check("alpha reply%=100%", ov["rows"][0][11] == "100.0%")
    check("last_activity not empty", ov["rows"][0][17] != "—")
    ...
    # Brand-new: 4th sheet (Daily Breakdown)
    ds = p["sheets"][3]
    check("daily has 4 sheets total", len(p["sheets"]) == 4)
    check("daily title is Daily Breakdown", ds["title"] == "Daily Breakdown")
    check("daily has settings metadata", "Settings:" in ds.get("notes", {}).get("A1", ""))
    check("daily has rows for alpha (only active campaign)", len(ds["rows"]) == 1)
    check("daily row day is today", ds["rows"][0][1] is not None)
    check("daily row interested=1", ds["rows"][0][2] == 1)
    check("daily row sent=3", ds["rows"][0][4] == 3)
    check("daily row bounced=1", ds["rows"][0][6] == 1)
    check("daily metadata in overview too", "Settings:" in p["sheets"][0].get("notes", {}).get("A1", ""))
    check("daily metadata in funnels too", "Settings:" in p["sheets"][1].get("notes", {}).get("A1", ""))
    check("daily metadata in sentiment too", "Settings:" in p["sheets"][2].get("notes", {}).get("A1", ""))
    ...
    ov = p["sheets"][0]
    row = ov["rows"][0]
    check("total_replies=2", row[8] == 2, f"got {row[8]}")
    check("ooo=1", row[9] == 1, f"got {row[9]}")
    check("human=1", row[10] == 1, f"got {row[10]}")
    check("reply%=total/base", row[11], f"got {row[11]}")  # should be non-empty string
    conn3.close()
    tmp3.cleanup()
    ...
    ov = p["sheets"][0]
    row = ov["rows"][0]
    check("LI-only: sent=0 (mdash)", row[4] == "—", f"got {row[4]}")
    check("LI-only: delivered mdash", row[5] == "—", f"got {row[5]}")
    check("LI-only: li_connects=1", row[12] == 1, f"got {row[12]}")
    check("LI-only: li_accepts mdash when not in rollup", row[13] == "—", f"got {row[13]}")
    check("LI-only: li_messages=1", row[15] == 1, f"got {row[15]}")
    check("LI-only: li_replies=1", row[16] == 1, f"got {row[16]}")
    ...
    ov = p["sheets"][0]
    row = ov["rows"][0]
    check("sentiment-mix: interested=2", row[2] == 2, f"got {row[2]}")
    check("sentiment-mix: not_interested=2", row[3] == 2, f"got {row[3]}")
    ...
    ov = p["sheets"][0]
    row = ov["rows"][0]
    check("no-sentiment: interested mdash", row[2] == "—", f"got {row[2]}")
    check("no-sentiment: not_interested mdash", row[3] == "—", f"got {row[3]}")
    conn6.close()
    tmp6.cleanup()
    ...
    ov = p["sheets"][0]
    check("all-time: sent=1", ov["rows"][0][4] == 1)
    check("all-time: replies=1", ov["rows"][0][8] == 1)
    ...
    check("all-alias: sent=1", p["sheets"][0]["rows"][0][4] == 1)
    conn8.close()
    tmp8.cleanup()
    ...
    ov = p["sheets"][0]
    row = ov["rows"][0]
    check("all-bounce: sent=1", row[4] == 1)
    check("all-bounce: bounced=1", row[6] == 1)
    check("all-bounce: delivered mdash", row[5] == "—")
    check("all-bounce: bounce%=100%", row[7] == "100.0%", f"got {row[7]}")
    check("all-bounce: reply% mdash (div by 0)", row[11] == "—")
    conn9.close()
    tmp9.cleanup()
    ...
    check("json: 4 sheets", len(deserialized["sheets"]) == 4)
    check("json: overview title (no date prefix)", deserialized["sheets"][0]["title"] == "Campaign Overview")
    ...
    check("ooo-only: replies=1", row[8] == 1)
    check("ooo-only: ooo=1", row[9] == 1)
    check("ooo-only: human mdash", row[10] == "—")
    check("ooo-only: reply%=100%", row[11] == "100.0%", f"got {row[11]}")
    conn12.close()
    tmp12.cleanup()
