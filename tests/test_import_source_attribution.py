...
def test_import_profiles_cli_source_flag(tmp_path):
    ...
    init = subprocess.run(
        [sys.executable, str(SCRIPTS / "pipeline.py"), "init", "--agent", "cursor"],
        capture_output=True,
        text=True,
        env=env,
    )
    # init may fail if multi-agent directories exist locally; that's env-specific
    if init.returncode != 0 and "multiple agent directories" in init.stderr:
        pytest.skip("Multi-agent environment, skipping")
