...
def test_publish_outreachmagic_includes_install_helpers():
    text = (ROOT / ".github/workflows/publish-platforms.yml").read_text(encoding="utf-8")
    assert "install-companions.sh" in text
    assert "skill-suite.json" in text
    assert "skill_suite.py" in text
